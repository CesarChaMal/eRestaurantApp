...
import com.erestaurant.erestaurantapp.config.KafkaSseConsumer;
import com.erestaurant.erestaurantapp.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/e-restaurant-app-kafka")
public class ERestaurantAppKafkaResource {

    private final Logger log = LoggerFactory.getLogger(ERestaurantAppKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public ERestaurantAppKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
