...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.CustomerProfile}.
 */
@RestController
@RequestMapping("/api")
public class CustomerProfileResource {
    ...
    /**
     * {@code SEARCH  /_search/customer-profiles?query=:query} : search for the customerProfile corresponding
     * to the query.
     *
    ...
    public Mono<List<CustomerProfileDTO>> searchCustomerProfiles(@RequestParam String query) {
        log.debug("REST request to search CustomerProfiles for query {}", query);
        return customerProfileService.search(query).collectList();
    }
}
