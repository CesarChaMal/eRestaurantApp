...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.Refunded}.
 */
@RestController
@RequestMapping("/api")
public class RefundedResource {
    ...
    private static final String ENTITY_NAME = "refunded";
    ...
    /**
     * {@code SEARCH  /_search/refundeds?query=:query} : search for the refunded corresponding
     * to the query.
     *
     * @param query the query of the refunded search.
     * @return the result of the search.
     */
    @GetMapping("/_search/refundeds")
    public Mono<List<RefundedDTO>> searchRefundeds(@RequestParam String query) {
        log.debug("REST request to search Refundeds for query {}", query);
        return refundedService.search(query).collectList();
    }
}
