...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.Complete}.
 */
@RestController
@RequestMapping("/api")
public class CompleteResource {
    ...
    private static final String ENTITY_NAME = "complete";
    ...
    /**
     * {@code SEARCH  /_search/completes?query=:query} : search for the complete corresponding
     * to the query.
     *
     * @param query the query of the complete search.
     * @return the result of the search.
     */
    @GetMapping("/_search/completes")
    public Mono<List<CompleteDTO>> searchCompletes(@RequestParam String query) {
        log.debug("REST request to search Completes for query {}", query);
        return completeService.search(query).collectList();
    }
}
