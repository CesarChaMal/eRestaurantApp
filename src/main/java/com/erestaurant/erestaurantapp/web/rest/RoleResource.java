...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.Role}.
 */
@RestController
@RequestMapping("/api")
public class RoleResource {
     ...
     */
    @GetMapping("/roles")
    public Mono<List<RoleDTO>> getAllRoles(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
    ...
    public Mono<List<RoleDTO>> searchRoles(@RequestParam String query) {
        log.debug("REST request to search Roles for query {}", query);
        return roleService.search(query).collectList();
    }
}
