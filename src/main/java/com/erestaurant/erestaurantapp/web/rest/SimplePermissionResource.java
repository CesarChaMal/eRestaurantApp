...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.SimplePermission}.
 */
@RestController
@RequestMapping("/api")
public class SimplePermissionResource {
    ...
    private static final String ENTITY_NAME = "simplePermission";
    ...
    /**
     * {@code SEARCH  /_search/simple-permissions?query=:query} : search for the simplePermission corresponding
     * to the query.
     *
     * @param query the query of the simplePermission search.
     * @return the result of the search.
     */
    @GetMapping("/_search/simple-permissions")
    public Mono<List<SimplePermissionDTO>> searchSimplePermissions(@RequestParam String query) {
        log.debug("REST request to search SimplePermissions for query {}", query);
        return simplePermissionService.search(query).collectList();
    }
}
