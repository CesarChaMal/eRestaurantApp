...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.Cancel}.
 */
@RestController
@RequestMapping("/api")
public class CancelResource {
    ...
    private static final String ENTITY_NAME = "cancel";
    ...
    /**
     * {@code SEARCH  /_search/cancels?query=:query} : search for the cancel corresponding
     * to the query.
     *
     * @param query the query of the cancel search.
     * @return the result of the search.
     */
    @GetMapping("/_search/cancels")
    public Mono<List<CancelDTO>> searchCancels(@RequestParam String query) {
        log.debug("REST request to search Cancels for query {}", query);
        return cancelService.search(query).collectList();
    }
}
