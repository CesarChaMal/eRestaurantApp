...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.NewOrder}.
 */
@RestController
@RequestMapping("/api")
public class NewOrderResource {
    ...
    private static final String ENTITY_NAME = "newOrder";
    ...
    /**
     * {@code SEARCH  /_search/new-orders?query=:query} : search for the newOrder corresponding
     * to the query.
     *
     * @param query the query of the newOrder search.
     * @return the result of the search.
     */
    @GetMapping("/_search/new-orders")
    public Mono<List<NewOrderDTO>> searchNewOrders(@RequestParam String query) {
        log.debug("REST request to search NewOrders for query {}", query);
        return newOrderService.search(query).collectList();
    }
}
