...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.AppDiscount}.
 */
@RestController
@RequestMapping("/api")
public class AppDiscountResource {
    ...
    private static final String ENTITY_NAME = "appDiscount";
    ...
    /**
     * {@code SEARCH  /_search/app-discounts?query=:query} : search for the appDiscount corresponding
     * to the query.
     *
     * @param query the query of the appDiscount search.
     * @return the result of the search.
     */
    @GetMapping("/_search/app-discounts")
    public Mono<List<AppDiscountDTO>> searchAppDiscounts(@RequestParam String query) {
        log.debug("REST request to search AppDiscounts for query {}", query);
        return appDiscountService.search(query).collectList();
    }
}
