...
/**
 * REST controller for managing {@link com.erestaurant.erestaurantapp.domain.OnHold}.
 */
@RestController
@RequestMapping("/api")
public class OnHoldResource {
    ...
    private static final String ENTITY_NAME = "onHold";
    ...
    /**
     * {@code SEARCH  /_search/on-holds?query=:query} : search for the onHold corresponding
     * to the query.
     *
     * @param query the query of the onHold search.
     * @return the result of the search.
     */
    @GetMapping("/_search/on-holds")
    public Mono<List<OnHoldDTO>> searchOnHolds(@RequestParam String query) {
        log.debug("REST request to search OnHolds for query {}", query);
        return onHoldService.search(query).collectList();
    }
}
