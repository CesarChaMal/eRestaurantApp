...
public interface RestaurantAdRepository extends ReactiveCrudRepository<RestaurantAd, String>, RestaurantAdRepositoryInternal {
    @Query("SELECT * FROM restaurant_ad entity WHERE entity.restaurant_id = :id")
    Flux<RestaurantAd> findByRestaurant(String id);

    @Query("SELECT * FROM restaurant_ad entity WHERE entity.restaurant_id IS NULL")
    Flux<RestaurantAd> findAllWhereRestaurantIsNull();
