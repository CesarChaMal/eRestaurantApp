...
public interface CustomerRepository extends ReactiveCrudRepository<Customer, String>, CustomerRepositoryInternal {
    ...
    @Query("SELECT * FROM customer entity WHERE entity.profile_id = :id")
    Flux<Customer> findByProfile(String id);

    @Query("SELECT * FROM customer entity WHERE entity.profile_id IS NULL")
