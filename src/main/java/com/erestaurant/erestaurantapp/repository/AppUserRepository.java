...
public interface AppUserRepository extends ReactiveCrudRepository<AppUser, String>, AppUserRepositoryInternal {
    ...
    @Query(
        "SELECT entity.* FROM app_user entity JOIN rel_app_user__roles joinTable ON entity.id = joinTable.roles_id WHERE joinTable.roles_id = :id"
    )
    Flux<AppUser> findByRoles(String id);
