...
class NewOrderResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/new-orders";
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.NewOrderSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createNewOrder() throws Exception {
        ...
        when(mockNewOrderSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the NewOrder
        ...
        // Validate the NewOrder in Elasticsearch
        verify(mockNewOrderSearchRepository, times(1)).save(testNewOrder);
    }
    ...
    void createNewOrderWithExistingId() throws Exception {
        ...
        // Validate the NewOrder in Elasticsearch
        verify(mockNewOrderSearchRepository, times(0)).save(newOrder);
    }
    ...
    void putNewNewOrder() throws Exception {
        ...
        // Validate the NewOrder in Elasticsearch
        verify(mockNewOrderSearchRepository).save(testNewOrder);
    }
    ...
    void deleteNewOrder() {
        // Configure the mock search repository
        when(mockNewOrderSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockNewOrderSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        newOrder.setId(UUID.randomUUID().toString());
        newOrderRepository.save(newOrder).block();
        ...
        // Validate the NewOrder in Elasticsearch
        verify(mockNewOrderSearchRepository, times(1)).deleteById(newOrder.getId());
    }
    ...
    void searchNewOrder() {
        ...
        newOrder.setId(UUID.randomUUID().toString());
        newOrderRepository.save(newOrder).block();
        when(mockNewOrderSearchRepository.search("id:" + newOrder.getId())).thenReturn(Flux.just(newOrder));

        // Search the newOrder
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + newOrder.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
