...
class CategoriesResourceIT {
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.CategoriesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createCategories() throws Exception {
        ...
        when(mockCategoriesSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
    ...
    void putNewCategories() throws Exception {
        ...
        // Validate the Categories in Elasticsearch
        verify(mockCategoriesSearchRepository).save(testCategories);
    }
    ...
    void deleteCategories() {
        // Configure the mock search repository
        when(mockCategoriesSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockCategoriesSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        categories.setId(UUID.randomUUID().toString());
        categoriesRepository.save(categories).block();
    ...
    void searchCategories() {
        ...
        categories.setId(UUID.randomUUID().toString());
        categoriesRepository.save(categories).block();
        when(mockCategoriesSearchRepository.search("id:" + categories.getId())).thenReturn(Flux.just(categories));

        // Search the categories
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + categories.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
