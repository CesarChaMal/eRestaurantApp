...
import com.erestaurant.erestaurantapp.repository.search.AppUserSearchRepository;
import com.erestaurant.erestaurantapp.service.AppUserService;
import com.erestaurant.erestaurantapp.service.dto.AppUserDTO;
import com.erestaurant.erestaurantapp.service.mapper.AppUserMapper;
...
class AppUserResourceIT {
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.AppUserSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public static void deleteEntities(EntityManager em) {
        try {
            em.deleteAll("rel_app_user__roles").block();
    ...
    void createAppUser() throws Exception {
        ...
        when(mockAppUserSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the AppUser
    ...
    void deleteAppUser() {
        // Configure the mock search repository
        when(mockAppUserSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockAppUserSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        appUser.setId(UUID.randomUUID().toString());
        appUserRepository.save(appUser).block();
    ...
    void searchAppUser() {
        // Configure the mock search repository
        when(mockAppUserSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockAppUserSearchRepository.count()).thenReturn(Mono.just(1L));
        // Initialize the database
        appUser.setId(UUID.randomUUID().toString());
        appUserRepository.save(appUser).block();
        when(mockAppUserSearchRepository.search("id:" + appUser.getId(), PageRequest.of(0, 20))).thenReturn(Flux.just(appUser));
