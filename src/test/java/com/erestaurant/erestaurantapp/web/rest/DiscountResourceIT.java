...
class DiscountResourceIT {
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.DiscountSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createDiscount() throws Exception {
        ...
        when(mockDiscountSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the Discount
    ...
    void putNewDiscount() throws Exception {
        ...
        // Validate the Discount in Elasticsearch
        verify(mockDiscountSearchRepository).save(testDiscount);
    }
    ...
    void deleteDiscount() {
        // Configure the mock search repository
        when(mockDiscountSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockDiscountSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        discount.setId(UUID.randomUUID().toString());
        discountRepository.save(discount).block();
    ...
    void searchDiscount() {
        ...
        discount.setId(UUID.randomUUID().toString());
        discountRepository.save(discount).block();
        when(mockDiscountSearchRepository.search("id:" + discount.getId())).thenReturn(Flux.just(discount));

        // Search the discount
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + discount.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
