...
class CustomerProfileResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/customer-profiles";
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.CustomerProfileSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createCustomerProfile() throws Exception {
        ...
        when(mockCustomerProfileSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        ...
        verify(mockCustomerProfileSearchRepository, times(1)).save(testCustomerProfile);
    }
    ...
    void createCustomerProfileWithExistingId() throws Exception {
        ...
        // Validate the CustomerProfile in Elasticsearch
        verify(mockCustomerProfileSearchRepository, times(0)).save(customerProfile);
    }
    ...
    void putNewCustomerProfile() throws Exception {
        ...
        // Validate the CustomerProfile in Elasticsearch
        verify(mockCustomerProfileSearchRepository).save(testCustomerProfile);
    }
    ...
    void deleteCustomerProfile() {
        // Configure the mock search repository
        when(mockCustomerProfileSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockCustomerProfileSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        customerProfile.setId(UUID.randomUUID().toString());
        customerProfileRepository.save(customerProfile).block();
        ...
        // Validate the CustomerProfile in Elasticsearch
        verify(mockCustomerProfileSearchRepository, times(1)).deleteById(customerProfile.getId());
    }
    ...
    void searchCustomerProfile() {
        ...
        customerProfile.setId(UUID.randomUUID().toString());
        customerProfileRepository.save(customerProfile).block();
        when(mockCustomerProfileSearchRepository.search("id:" + customerProfile.getId())).thenReturn(Flux.just(customerProfile));

        // Search the customerProfile
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + customerProfile.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
