...
class SimplePermissionResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/simple-permissions";
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.SimplePermissionSearchRepositoryMockConfiguration
     */
    @Autowired
    private SimplePermissionSearchRepository mockSimplePermissionSearchRepository;
    ...
    void createSimplePermission() throws Exception {
        ...
        when(mockSimplePermissionSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        ...
        // Validate the SimplePermission in Elasticsearch
        verify(mockSimplePermissionSearchRepository, times(1)).save(testSimplePermission);
    }
    ...
    void createSimplePermissionWithExistingId() throws Exception {
        ...
        // Validate the SimplePermission in Elasticsearch
        verify(mockSimplePermissionSearchRepository, times(0)).save(simplePermission);
    }
    ...
    void putNewSimplePermission() throws Exception {
        ...
        // Validate the SimplePermission in Elasticsearch
        verify(mockSimplePermissionSearchRepository).save(testSimplePermission);
    }
    ...
    void deleteSimplePermission() {
        // Configure the mock search repository
        when(mockSimplePermissionSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockSimplePermissionSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        simplePermission.setId(UUID.randomUUID().toString());
        simplePermissionRepository.save(simplePermission).block();
        ...
        // Validate the SimplePermission in Elasticsearch
        verify(mockSimplePermissionSearchRepository, times(1)).deleteById(simplePermission.getId());
    }

    @Test
    void searchSimplePermission() {
        // Configure the mock search repository
        when(mockSimplePermissionSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Initialize the database
        simplePermission.setId(UUID.randomUUID().toString());
        simplePermissionRepository.save(simplePermission).block();
        when(mockSimplePermissionSearchRepository.search("id:" + simplePermission.getId())).thenReturn(Flux.just(simplePermission));

        // Search the simplePermission
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + simplePermission.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
