...
class AppDiscountResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/app-discounts";
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.AppDiscountSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createAppDiscount() throws Exception {
        ...
        when(mockAppDiscountSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        ...
        // Validate the AppDiscount in Elasticsearch
        verify(mockAppDiscountSearchRepository, times(1)).save(testAppDiscount);
    }
    ...
    void createAppDiscountWithExistingId() throws Exception {
        ...
        // Validate the AppDiscount in Elasticsearch
        verify(mockAppDiscountSearchRepository, times(0)).save(appDiscount);
    }
    ...
    void putNewAppDiscount() throws Exception {
        ...
        // Validate the AppDiscount in Elasticsearch
        verify(mockAppDiscountSearchRepository).save(testAppDiscount);
    }
    ...
    void deleteAppDiscount() {
        // Configure the mock search repository
        when(mockAppDiscountSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockAppDiscountSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        appDiscount.setId(UUID.randomUUID().toString());
        appDiscountRepository.save(appDiscount).block();
        ...
        // Validate the AppDiscount in Elasticsearch
        verify(mockAppDiscountSearchRepository, times(1)).deleteById(appDiscount.getId());
    }

    @Test
    void searchAppDiscount() {
        // Configure the mock search repository
        when(mockAppDiscountSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Initialize the database
        appDiscount.setId(UUID.randomUUID().toString());
        appDiscountRepository.save(appDiscount).block();
        when(mockAppDiscountSearchRepository.search("id:" + appDiscount.getId())).thenReturn(Flux.just(appDiscount));

        // Search the appDiscount
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + appDiscount.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
