...
class OrderResourceIT {
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.OrderSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createOrder() throws Exception {
        ...
        when(mockOrderSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the Order
    ...
    void deleteOrder() {
        // Configure the mock search repository
        when(mockOrderSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockOrderSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        order.setId(UUID.randomUUID().toString());
        orderRepository.save(order).block();
    ...
    void searchOrder() {
        ...
        order.setId(UUID.randomUUID().toString());
        orderRepository.save(order).block();
        when(mockOrderSearchRepository.search("id:" + order.getId())).thenReturn(Flux.just(order));
