...
import com.erestaurant.erestaurantapp.repository.CartRepository;
import com.erestaurant.erestaurantapp.repository.EntityManager;
import com.erestaurant.erestaurantapp.repository.search.CartSearchRepository;
import com.erestaurant.erestaurantapp.service.dto.CartDTO;
import com.erestaurant.erestaurantapp.service.mapper.CartMapper;
...
class CartResourceIT {
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.CartSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void deleteCart() {
        // Configure the mock search repository
        when(mockCartSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockCartSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        cart.setId(UUID.randomUUID().toString());
        cartRepository.save(cart).block();
