...
class OnHoldResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/on-holds";
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.OnHoldSearchRepositoryMockConfiguration
     */
    @Autowired
    private OnHoldSearchRepository mockOnHoldSearchRepository;
    ...
    void createOnHold() throws Exception {
        ...
        when(mockOnHoldSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the OnHold
        ...
        // Validate the OnHold in Elasticsearch
        verify(mockOnHoldSearchRepository, times(1)).save(testOnHold);
    }
    ...
    void createOnHoldWithExistingId() throws Exception {
        ...
        // Validate the OnHold in Elasticsearch
        verify(mockOnHoldSearchRepository, times(0)).save(onHold);
    }
    ...
    void putNewOnHold() throws Exception {
        ...
        // Validate the OnHold in Elasticsearch
        verify(mockOnHoldSearchRepository).save(testOnHold);
    }
    ...
    void deleteOnHold() {
        // Configure the mock search repository
        when(mockOnHoldSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockOnHoldSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        onHold.setId(UUID.randomUUID().toString());
        onHoldRepository.save(onHold).block();
        ...
        // Validate the OnHold in Elasticsearch
        verify(mockOnHoldSearchRepository, times(1)).deleteById(onHold.getId());
    }
    ...
    void searchOnHold() {
        ...
        onHold.setId(UUID.randomUUID().toString());
        onHoldRepository.save(onHold).block();
        when(mockOnHoldSearchRepository.search("id:" + onHold.getId())).thenReturn(Flux.just(onHold));

        // Search the onHold
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + onHold.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
