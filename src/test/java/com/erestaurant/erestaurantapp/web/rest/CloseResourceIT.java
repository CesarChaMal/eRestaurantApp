...
class CloseResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/closes";
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.CloseSearchRepositoryMockConfiguration
     */
    @Autowired
    private CloseSearchRepository mockCloseSearchRepository;
    ...
    void createClose() throws Exception {
        ...
        when(mockCloseSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the Close
        ...
        // Validate the Close in Elasticsearch
        verify(mockCloseSearchRepository, times(1)).save(testClose);
    }
    ...
    void createCloseWithExistingId() throws Exception {
        ...
        // Validate the Close in Elasticsearch
        verify(mockCloseSearchRepository, times(0)).save(close);
    }
    ...
    void putNewClose() throws Exception {
        ...
        // Validate the Close in Elasticsearch
        verify(mockCloseSearchRepository).save(testClose);
    }
    ...
    void deleteClose() {
        // Configure the mock search repository
        when(mockCloseSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockCloseSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        close.setId(UUID.randomUUID().toString());
        closeRepository.save(close).block();
        ...
        // Validate the Close in Elasticsearch
        verify(mockCloseSearchRepository, times(1)).deleteById(close.getId());
    }
    ...
    void searchClose() {
        ...
        close.setId(UUID.randomUUID().toString());
        closeRepository.save(close).block();
        when(mockCloseSearchRepository.search("id:" + close.getId())).thenReturn(Flux.just(close));

        // Search the close
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + close.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
