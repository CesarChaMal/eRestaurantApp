...
class CancelResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/cancels";
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.CancelSearchRepositoryMockConfiguration
     */
    @Autowired
    private CancelSearchRepository mockCancelSearchRepository;
    ...
    void createCancel() throws Exception {
        ...
        when(mockCancelSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the Cancel
        ...
        // Validate the Cancel in Elasticsearch
        verify(mockCancelSearchRepository, times(1)).save(testCancel);
    }
    ...
    void createCancelWithExistingId() throws Exception {
        ...
        // Validate the Cancel in Elasticsearch
        verify(mockCancelSearchRepository, times(0)).save(cancel);
    }
    ...
    void putNewCancel() throws Exception {
        ...
        // Validate the Cancel in Elasticsearch
        verify(mockCancelSearchRepository).save(testCancel);
    }
    ...
    void deleteCancel() {
        // Configure the mock search repository
        when(mockCancelSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockCancelSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        cancel.setId(UUID.randomUUID().toString());
        cancelRepository.save(cancel).block();
        ...
        // Validate the Cancel in Elasticsearch
        verify(mockCancelSearchRepository, times(1)).deleteById(cancel.getId());
    }
    ...
    void searchCancel() {
        ...
        cancel.setId(UUID.randomUUID().toString());
        cancelRepository.save(cancel).block();
        when(mockCancelSearchRepository.search("id:" + cancel.getId())).thenReturn(Flux.just(cancel));

        // Search the cancel
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + cancel.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
