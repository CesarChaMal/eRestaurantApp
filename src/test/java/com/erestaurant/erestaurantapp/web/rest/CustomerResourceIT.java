...
class CustomerResourceIT {
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.CustomerSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void deleteCustomer() {
        // Configure the mock search repository
        when(mockCustomerSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockCustomerSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        customer.setId(UUID.randomUUID().toString());
        customerRepository.save(customer).block();
    ...
    void searchCustomer() {
        // Configure the mock search repository
        when(mockCustomerSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockCustomerSearchRepository.count()).thenReturn(Mono.just(1L));
        // Initialize the database
        customer.setId(UUID.randomUUID().toString());
        customerRepository.save(customer).block();
        when(mockCustomerSearchRepository.search("id:" + customer.getId(), PageRequest.of(0, 20))).thenReturn(Flux.just(customer));
