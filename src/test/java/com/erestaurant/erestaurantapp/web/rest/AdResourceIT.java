...
class AdResourceIT {
     ...
     *
     * @see com.erestaurant.erestaurantapp.repository.search.AdSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createAd() throws Exception {
        ...
        when(mockAdSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        // Create the Ad
    ...
    void deleteAd() {
        // Configure the mock search repository
        when(mockAdSearchRepository.save(any())).thenAnswer(invocation -> Mono.just(invocation.getArgument(0)));
        when(mockAdSearchRepository.deleteById(anyString())).thenReturn(Mono.empty());
        // Initialize the database
        ad.setId(UUID.randomUUID().toString());
        adRepository.save(ad).block();
    ...
    void searchAd() {
        ...
        ad.setId(UUID.randomUUID().toString());
        adRepository.save(ad).block();
        when(mockAdSearchRepository.search("id:" + ad.getId())).thenReturn(Flux.just(ad));

        // Search the ad
        webTestClient
            .get()
            .uri(ENTITY_SEARCH_API_URL + "?query=id:" + ad.getId())
            .exchange()
            .expectStatus()
            .isOk()
            .expectHeader()
